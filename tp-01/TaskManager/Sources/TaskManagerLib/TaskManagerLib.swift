...
public func createCorrectTaskManager() -> PTNet {
    ...
    let processPool = PTPlace(named: "processPool")
    let inProgress  = PTPlace(named: "inProgress")
    let complementaryPlace = PTPlace(named: "complementaryPlace") // ajouter une nouvelle place
    ...
    let success     = PTTransition(
        named          : "success",
        preconditions  : [PTArc(place: taskPool), PTArc(place: inProgress), PTArc(place: complementaryPlace)],//rajouter la pré condition à success
        postconditions : [])
    let exec       = PTTransition(
        named          : "exec",
        preconditions  : [PTArc(place: taskPool), PTArc(place: processPool)],
        postconditions : [PTArc(place: taskPool), PTArc(place: inProgress), PTArc(place: complementaryPlace)])//rajouter un postcondition de exec à la plaPlaceComplimantaire
    let fail        = PTTransition(
        named          : "fail",
        preconditions  : [PTArc(place: inProgress), PTArc(place: complementaryPlace)],//rajouter la pré condition à fail
        postconditions : [])
    
    // P/T-net
    return PTNet(
        places: [taskPool, processPool, inProgress, complementaryPlace],
        transitions: [create, spawn, success, exec, fail])
}
